...
export const HeroGridCard = ({hero} : Props) => {
  ...
  const { id, image, alias, status, universe, name, team, category, slug,
    description, strength, durability, speed, intelligence, powers, firstAppearance
  } = hero
  ...
  return (
    ...
    >
      <div className="relative h-64">
        ...
        <Badge 
          className={`absolute top-3 right-3 text-xs ${universe === 'DC' ? 'bg-blue-600' : 'bg-red-600'} text-white`}
        >{universe}</Badge>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            {powers.slice(0,3).map(p => (
              <Badge variant="outline" className="text-xs" key={p}>
                {p}
              </Badge>
            ))}
            {powers.length > 3 && (
